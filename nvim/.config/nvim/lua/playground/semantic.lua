...
local has_bins = function(...)
    for i = 1, select('#', ...) do
        if 0 == fn.executable((select(i, ...))) then return false end
    end
    return true
end
...
M.generate = function()
    if not has_bins(metac, metap) then
        ...
        local create_cmd = string.format("%s -d %s %s", metac,
                                         semantic_cache_dir, current_file)
        local created = fn.system(create_cmd)
        if not (created == "") then
            print("Couldn't create semanticdb")
        else
            local semanticdb_path = path_join(semantic_cache_dir, "META-INF",
                                              "semanticdb",
